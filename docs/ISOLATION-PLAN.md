...
[`ACT-INTERFACE-MAP.md`](ACT-INTERFACE-MAP.md) (the upstream legacy contract each satellite facade
reproduces), [`PLUGIN-API.md`](PLUGIN-API.md) (the host pipes). The parity/oracle harnesses the gates
build on are [`tools/mass-compare`](../tools/mass-compare/README.md) (corpus-scale) and the
...
4. **The host knows no plugin.** Runtime routing is keyed by stream and capability names, never
   ...
   see [`ACT-INTERFACE-MAP.md`](ACT-INTERFACE-MAP.md)), (b) the `Fct.App` install-catalog UI, and (c) the
...
- **Consumer projection (new, inside each satellite):** the facade serves its plugin's reads
  ...
  exactly as under real ACT ([`ARCHITECTURE.md`](ARCHITECTURE.md) §5).
...
- **P0 — Doctrine & documentation: ✅ done.** Invariants + topology (§1–§3) recorded here and aligned
  across `CLAUDE.md`/`ARCHITECTURE.md`/`ACT-INTERFACE-MAP.md`/`PLUGIN-API.md`.
...
- [ ] **Live-game scope recorded:** Sharlayan reads, WPF overlay rendering, and spell-timer
      display need a live FFXIV process/desktop session — they are manual live-game acceptance
      (A1), not CI. CI proves load/attach/route; A1 proves pixels.
...
- [ ] Doc truth-up: `CLAUDE.md` "Hojoring isolation is deferred to P10" → present-tense
      isolated; the test matrix extended to the five-package set; this document's
      checkboxes.
